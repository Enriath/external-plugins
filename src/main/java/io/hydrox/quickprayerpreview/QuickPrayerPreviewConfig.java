package io.hydrox.quickprayerpreview;

import java.awt.Color;
import java.awt.event.InputEvent;
import java.awt.event.KeyEvent;
import net.runelite.client.config.Config;
import net.runelite.client.config.ConfigGroup;
import net.runelite.client.config.ConfigItem;
import net.runelite.client.config.ModifierlessKeybind;
import net.runelite.client.config.Range;
import net.runelite.client.config.Units;

@ConfigGroup(QuickPrayerPreviewConfig.CONFIG_GROUP)
public interface QuickPrayerPreviewConfig extends Config
{
	String CONFIG_GROUP = "quickprayerpreview";

	String KEY_TOOLTIP_SPRITE_SIZE = "tooltipSpriteSize";

	@ConfigItem(
		name = "Replace Tooltip Sprite With Text",
		description = "Replaces the tooltip prayer sprites with text.",
		position = 0,
		keyName = "useTextTooltip"
	)
	default boolean useTextTooltip()
	{
		return false;
	}

	@ConfigItem(
		name = "Tooltip Text Color",
		description = "Color of the tooltip text, if enabled.",
		position = 1,
		keyName = "tooltipTextColor"
	)
	default Color tooltipTextColor()
	{
		return Color.WHITE;
	}

	@ConfigItem(
		name = "Tooltip Sprite Size",
		description = "Size of tooltip prayer sprites.",
		position = 2,
		keyName = KEY_TOOLTIP_SPRITE_SIZE
	)
	@Units(Units.PIXELS)
	@Range(min = 8, max = 32)
	default int tooltipSpriteSize()
	{
		return 32;
	}

	@ConfigItem(
		name = "Require Key Press",
		description = "Only display quick-prayer orb tooltip while pressing the set key.",
		position = 3,
		keyName = "requireKeyPress"
	)
	default boolean requireKeyPress()
	{
		return false;
	}

	@ConfigItem(
		name = "Key To Press",
		description = "The key to press to display the quick-prayer orb tooltip.",
		position = 4,
		keyName = "keyToPress"
	)
	default ModifierlessKeybind keyToPress()
	{
		return new ModifierlessKeybind(KeyEvent.VK_SHIFT, InputEvent.SHIFT_DOWN_MASK);
	}

	@ConfigItem(
		name = "Add Print Menu Entry",
		description = "Adds a menu entry to the quick-prayer orb to print to chatbox.",
		position = 5,
		keyName = "addPrintMenuEntry"
	)
	default boolean addPrintMenuEntry()
	{
		return false;
	}
}
